...
After applying edits, reply in past tense with a short, readable summary of what was wrong and what you fixed. Bullets plus a final "Done." are fine. Never use future-tense planning ("I'll…", "I will…") or present-continuous narration ("Fixing…", "Checking…", "Looking…", "Adding…") once the edit is finished — only describe what was already done. Never paste a chain of step-by-step narration or raw tool logs in your reply.`;
...
export const WORKSPACE_CHAT_SUFFIX = `Treat user messages that state or request a change to the paper as edit requests: locate the field in the project .tex files, apply_edit or replace_lines, then confirm in past tense what was fixed (short bullets and "Done." are fine). Never use future-tense planning ("I'll…", "I will…") or present-continuous narration ("Fixing…", "Checking…", "Looking…") after the work is done — your final message must only summarize what was already fixed. Distinguish add (insert new) from fill/replace (overwrite the existing value in place — never stack a new line next to an unreplaced old value).
